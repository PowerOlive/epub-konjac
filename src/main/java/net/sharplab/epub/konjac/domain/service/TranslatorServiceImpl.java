...
public class TranslatorServiceImpl implements TranslatorService{
     ...
     * インライン要素のタグリスト
     */
    private final static List<String> INLINE_ELEMENT_NAMES = Arrays.asList(
    ...
    /**
     * Constructor
     * @param msTranslatorRestRepository RestRepository for calling Microsoft Translator
     */
    public TranslatorServiceImpl(MSTranslatorRestRepository msTranslatorRestRepository){
        this.msTranslatorRestRepository = msTranslatorRestRepository;
    }
     ...
     */
    @Override
    public String translateString(String xmlString, TranslatorOption translatorOption){
        Document document = XmlUtils.parseXmlStringToDocument(xmlString);
        Document translatedDocument = translateDocument(document, translatorOption);
     ...
     */
    @Override
    public Document translateDocument(Document document, TranslatorOption translatorOption) {
        ...
        replaceWithTranslatedText(document, translateRequestChunks, translatorOption);
    ...
    List<TranslateRequestChunk> formTranslateRequestChunks(List<TranslateRequest> translateRequests){
        ...
        TranslateRequestChunk lastTranslateRequestChunk = new TranslateRequestChunk();
        translateRequestChunks.add(lastTranslateRequestChunk);
        for(TranslateRequest translateRequest : translateRequests) {
            ...
            if(textLengthCounter + sourceXmlString.length() > MAX_REQUESTABLE_TEXT_LENGTH){
                lastTranslateRequestChunk = new TranslateRequestChunk();
                translateRequestChunks.add(lastTranslateRequestChunk);
                textLengthCounter = 0;
            }
            lastTranslateRequestChunk.getTranslateRequests().add(translateRequest);
     ...
     */
    void replaceWithTranslatedText(Document document, List<TranslateRequestChunk> translateRequestChunks, TranslatorOption translatorOption) {
        translateRequestChunks.forEach(translateRequestChunk -> {
            ...
            List<Text> texts = translateRequests.stream().map(item -> new Text(item.getSourceXmlString())).collect(Collectors.toList());
            TranslateArrayRequest translateArrayRequest = new TranslateArrayRequest("", translatorOption.getSrcLang(), translatorOption.getDstLang(), options, texts);
            TranslateArrayResponse[] translateArrayResponses = msTranslatorRestRepository.translate(translateArrayRequest);
            for(int i = 0; i< translateRequests.size(); i++){
                ...
                DocumentFragment documentFragment = XmlUtils.parseXmlStringToDocumentFragment(document, translateArrayResponses[i].getTranslatedText());
    ...
    private boolean isExcludedNode(Node node){
        return EXCLUDED_ELEMENT_NAMES.contains(node.getNodeName());
    }
